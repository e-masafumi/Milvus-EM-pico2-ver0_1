/*!
 * @file Adafruit_BNO055_raspico.cpp
 *
...
#include "Adafruit_BNO055_raspico_raspico.h"

/*!
 *  @brief  Instantiates a new Adafruit_BNO055_raspico class
 *  @param  sensorID
 *          sensor ID
 *  @param  address
 *          i2c address
 ...
 *          Wire object
 */
Adafruit_BNO055_raspico::Adafruit_BNO055_raspico(int32_t sensorID, uint8_t address,
                                 TwoWire *theWire) {
 ...
 */
bool Adafruit_BNO055_raspico::begin(adafruit_bno055_opmode_t mode) {
 ...
 *            OPERATION_MODE_NDOF]
 */
void Adafruit_BNO055_raspico::setMode(adafruit_bno055_opmode_t mode) {
  _mode = mode;
 ...
 */
adafruit_bno055_opmode_t Adafruit_BNO055_raspico::getMode() {
 ...
 *           REMAP_CONFIG_P7]
 */
void Adafruit_BNO055_raspico::setAxisRemap(
 ...
 *           REMAP_SIGN_P7]
 */
void Adafruit_BNO055_raspico::setAxisSign(adafruit_bno055_axis_remap_sign_t remapsign) {
  adafruit_bno055_opmode_t modeback = _mode;
 ...
 */
void Adafruit_BNO055_raspico::setExtCrystalUse(boolean usextal) {
  adafruit_bno055_opmode_t modeback = _mode;
 ...
 *           system error info
 */
void Adafruit_BNO055_raspico::getSystemStatus(uint8_t *system_status,
 ...
 *          revision info
 */
void Adafruit_BNO055_raspico::getRevInfo(adafruit_bno055_rev_info_t *info) {
  uint8_t a, b;
 ...
 */
void Adafruit_BNO055_raspico::getCalibration(uint8_t *sys, uint8_t *gyro,
 ...
 */
int8_t Adafruit_BNO055_raspico::getTemp() {
 ...
 */
imu::Vector<3> Adafruit_BNO055_raspico::getVector(adafruit_vector_type_t vector_type) {
  imu::Vector<3> xyz;
  uint8_t buffer[6];
  memset(buffer, 0, 6);
 ...
 */
imu::Quaternion Adafruit_BNO055_raspico::getQuat() {
  uint8_t buffer[8];
  memset(buffer, 0, 8);
 ...
 *          Sensor description
 */
void Adafruit_BNO055_raspico::getSensor(sensor_t *sensor) {
 ...
 */
bool Adafruit_BNO055_raspico::getEvent(sensors_event_t *event) {
  /* Clear the event */
  ...
  imu::Vector<3> euler = getVector(Adafruit_BNO055_raspico::VECTOR_EULER);
...
bool Adafruit_BNO055_raspico::getEvent(sensors_event_t *event,
  ...
  imu::Vector<3> vec;
  if (vec_type == Adafruit_BNO055_raspico::VECTOR_LINEARACCEL) {
    event->type = SENSOR_TYPE_LINEAR_ACCELERATION;
    vec = getVector(Adafruit_BNO055_raspico::VECTOR_LINEARACCEL);

    event->acceleration.x = vec.x();
    event->acceleration.y = vec.y();
    event->acceleration.z = vec.z();
  } else if (vec_type == Adafruit_BNO055_raspico::VECTOR_ACCELEROMETER) {
    event->type = SENSOR_TYPE_ACCELEROMETER;
    vec = getVector(Adafruit_BNO055_raspico::VECTOR_ACCELEROMETER);

    event->acceleration.x = vec.x();
    event->acceleration.y = vec.y();
    event->acceleration.z = vec.z();
  } else if (vec_type == Adafruit_BNO055_raspico::VECTOR_GRAVITY) {
    event->type = SENSOR_TYPE_GRAVITY;
    vec = getVector(Adafruit_BNO055_raspico::VECTOR_GRAVITY);

    event->acceleration.x = vec.x();
    event->acceleration.y = vec.y();
    event->acceleration.z = vec.z();
  } else if (vec_type == Adafruit_BNO055_raspico::VECTOR_EULER) {
    event->type = SENSOR_TYPE_ORIENTATION;
    vec = getVector(Adafruit_BNO055_raspico::VECTOR_EULER);

    event->orientation.x = vec.x();
    event->orientation.y = vec.y();
    event->orientation.z = vec.z();
  } else if (vec_type == Adafruit_BNO055_raspico::VECTOR_GYROSCOPE) {
    event->type = SENSOR_TYPE_GYROSCOPE;
    vec = getVector(Adafruit_BNO055_raspico::VECTOR_GYROSCOPE);

    event->gyro.x = vec.x() * SENSORS_DPS_TO_RADS;
    event->gyro.y = vec.y() * SENSORS_DPS_TO_RADS;
    event->gyro.z = vec.z() * SENSORS_DPS_TO_RADS;
  } else if (vec_type == Adafruit_BNO055_raspico::VECTOR_MAGNETOMETER) {
    event->type = SENSOR_TYPE_MAGNETIC_FIELD;
    vec = getVector(Adafruit_BNO055_raspico::VECTOR_MAGNETOMETER);
 ...
 */
bool Adafruit_BNO055_raspico::getSensorOffsets(uint8_t *calibData) {
 ...
 *  @return true if read is successful
 */
bool Adafruit_BNO055_raspico::getSensorOffsets(
 ...
 *          calibration data
 */
void Adafruit_BNO055_raspico::setSensorOffsets(const uint8_t *calibData) {
  adafruit_bno055_opmode_t lastMode = _mode;
  setMode(OPERATION_MODE_CONFIG);
  delay(25);
 ...
 */
void Adafruit_BNO055_raspico::setSensorOffsets(
 ...
 */
bool Adafruit_BNO055_raspico::isFullyCalibrated() {
 ...
 */
void Adafruit_BNO055_raspico::enterSuspendMode() {
  adafruit_bno055_opmode_t modeback = _mode;
 ...
 */
void Adafruit_BNO055_raspico::enterNormalMode() {
  adafruit_bno055_opmode_t modeback = _mode;
 ...
 */
bool Adafruit_BNO055_raspico::write8(adafruit_bno055_reg_t reg, byte value) {
 ...
 */
byte Adafruit_BNO055_raspico::read8(adafruit_bno055_reg_t reg) {
 ...
 */
bool Adafruit_BNO055_raspico::readLen(adafruit_bno055_reg_t reg, byte *buffer,
                              uint8_t len) {
